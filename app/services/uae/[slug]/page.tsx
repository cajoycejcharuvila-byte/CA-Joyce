...
export async function generateMetadata({ params }: PageProps) {
  ...
  return seoGenerateMetadata({
    ...
    path: `/services/uae/${slug}`,
    type: "website",
  });
}

export default async function UAEServiceDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const service = getServiceBySlug(slug);
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: service.title.replace(" (UAE)", ""), path: `/services/uae/${slug}` },
  ]);
  ...
  const allUAE = getUAEServices();
  const currentIndex = allUAE.findIndex((s) => s.slug === slug);
  
  const prevService = allUAE[currentIndex === 0 ? allUAE.length - 1 : currentIndex - 1];
  const nextService = allUAE[currentIndex === allUAE.length - 1 ? 0 : currentIndex + 1];

  const relatedServices = allUAE
          ...
          <aside className="lg:col-span-4 space-y-8 lg:sticky lg:top-[130px]">
            ...
            <div className="bg-white border border-brand-border rounded-[32px] p-8 shadow-soft">
              ...
              <p className="font-sans text-xs text-brand-secondary leading-relaxed mb-6">
                Connect with our advisory practice to manage FTA tax compliance registration and bookkeeping obligations.
              </p>
